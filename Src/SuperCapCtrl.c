 ...
 * 作者：魔法电容使者
 * 
 * V1.2.2608之后版本的代码通过修改宏定义，兼容Lite和Plus控制板。
...
static AnomalyDetectionTypeDef sOCPbatAnomalyConfig = {0};
...
static AnomalyDetectionTypeDef sBOOMAnomalyConfig = {0};
...
static const ADC_Fit_ParametersTypeDef csADC_Fit_Parameters[50] = {
    ...
    {0x00300057, 0x33354B02, 0x32333837, 0.0094f, 0.0680f, 0.0068f, 0.0079f, 0.0034f, 0.0237f, 0.0175f, -47.279f},//Lite
    ...
    {0x00500064, 0x484E5006, 0x20353031, 0.0099f, 0.1146f, 0.0100f, 0.0813f, 0.0078f, 0.0019f, -0.0158f, 21.98f},//Plus
...
static uint32_t PID_CurrentLoopOut_UVP = 0; // 中间量，电流环PID输出，在电容组欠压保护时
...
static float PowerLimint = 0; // 裁判系统下发的功率限制值(需要电控发过来)
...
static int32_t SuperCapPower_AVG;
...
inline void A_Timing_Ranking_Idea(void){
    ...
    //限制超级电容控制板从CAN中接收数据的刷新速率，避免过快刷新导致控制不稳定
...
inline void Free_Loop(void){
    ...
    //这两条是温度检测的ADC通道，使用的是规则组，因为其采样时间较长，故与电压电流独立进行，不用担心会影响电压电流的采样。
...
void State_Change(void){
    ...
    PowerLimint = DEBUG_MODE_POWER_LIMIT;
#else
    // 判断CAN是否离线。
    CAN_WDG_Counter++;
    ...
    Anomaly_Detection(&sOTPmosAnomalyConfig, Tmos, &sStateBit.OTP_MOS_bit);
    Anomaly_Detection(&sOTPcapAnomalyConfig, Tcap, &sStateBit.OTP_CAP_bit);

#ifdef PLUS
    Anomaly_Detection(&sBOOMAnomalyConfig, (float)BOOM, &sStateBit.BOOM_bit);
#endif
...
void Soft_Start_Loop(void){
    ...
    if (sStateBit.Enable_bit == ENABLE) {
...
/**
 * 保险丝熔断保护，进入此函数将无法退出，一般会伴随半桥爆炸，更换保险丝/半桥MOS之后恢复。
 * sStateBit.BOOM_bit =1
 * 无视电控的发过来的标志。
 * 发回去给电控的数据
 * sCAN_TX_data.SuperCapReady = 0
 * sCAN_TX_data.SuperCapState = 9
 */
...
------------------------------------------------------------------------------------------------------------------------------------------
*/
static uint32_t UVP_CapRecover_CNT = 0;
/**
 * 超电过放保护环路
 * sStateBit.UVP_Cap_bit =1
 * 电控发过来的信息必须为：
...
void CAP_Undervoltage_Protection_Loop(void){
    ...
    if (sStateBit.Enable_bit == ENABLE) {
        ...
        PID_Compute(&sPID_AutomaticCompensation, Pbat, PowerLimint, &PID_PowerLoopOut);
        ...
        if (IcapUvpLoopDownEgde == FALLING) {
            ...
            PID_Compute(&sPID_buck_I_UVP, Icap, 0, &PID_CurrentLoopOut_UVP);

            Loop_Competition_Boost(TIM1_PWM2_Comapre, PID_CurrentLoopOut_UVP, &TIM1_PWM2_Comapre);
...
static uint32_t OTP_RecoverDelay = 0;
/**
 * 过温保护函数
 * sStateBit.UVP_Cap_bit =1
 * 无视电控发过来的标志。
...
void Automatic_Power_Compensation_Loop(void){
    ...
    Hysteresis_Comparator(Pbat ,PowerLimint + 100, PowerLimint , &PbatOPLEdge);
...
static uint32_t PMOS_OffDelay = 0; //PMOS关断延迟
static uint32_t PMOS_OnDelay = 0; //PWM开通延迟
static uint8_t PMOS_ReOpen =0; //PMOS重新打开
static uint32_t PMOS_ReOpenDelay =0; //PMOS重新打开
...
static uint32_t LED_Refresh_CNT = 0; // 中间变量，用于同步灯光闪烁
/**
 * LED刷新函数，刷新频率大概为12Hz
...
void Power_Loop_Parameter_Init(void){
    ...
    sPID_buck_I_UVP.ControllerDirection = DIRECT; // 意思是，控制方向与变化方向相同
...
void Protection_Init(void){
    ...
    sOCPbatAnomalyConfig.SchmittTriggerDirection = RISING;
    sOCPbatAnomalyConfig.RisingThreshold = SOFTWARE_OCP_IBAT;
    sOCPbatAnomalyConfig.FallingThreshold = SOFTWARE_OCP_RECOVER_IBAT;
    ...
    sOVPbatAnomalyConfig.FallingThreshold = SOFTWARE_OVP_RECOVER_VBAT;
    sOVPbatAnomalyConfig.Timeout = COUNT_TO_200MS_ON_100Khz;
    Anomaly_Detection_Init(&sOVPbatAnomalyConfig);
    
    sUVPbatAnomalyConfig.SchmittTriggerDirection = FALLING;
    sUVPbatAnomalyConfig.RisingThreshold = SOFTWARE_UVP_BAT + 1;
    sUVPbatAnomalyConfig.FallingThreshold = SOFTWARE_UVP_BAT;
    sUVPbatAnomalyConfig.Timeout = COUNT_TO_5S_ON_100Khz;
    Anomaly_Detection_Init(&sUVPbatAnomalyConfig);
    
    sOTPcapAnomalyConfig.SchmittTriggerDirection = RISING;
    sOTPcapAnomalyConfig.RisingThreshold = SOFTWARE_OTP_CAP;
    sOTPcapAnomalyConfig.FallingThreshold = SOFTWARE_OTP_RECOVER_CAP;
    sOTPcapAnomalyConfig.Timeout = COUNT_TO_5S_ON_100Khz;
    Anomaly_Detection_Init(&sOTPcapAnomalyConfig);

    sOTPmosAnomalyConfig.SchmittTriggerDirection = RISING;
    sOTPmosAnomalyConfig.RisingThreshold = SOFTWARE_OTP_MOS;
    sOTPmosAnomalyConfig.FallingThreshold = SOFTWARE_OTP_RECOVER_MOS;
    sOTPmosAnomalyConfig.Timeout = COUNT_TO_5S_ON_100Khz;
    Anomaly_Detection_Init(&sOTPmosAnomalyConfig);

    sBOOMAnomalyConfig.SchmittTriggerDirection = RISING;
    sBOOMAnomalyConfig.RisingThreshold = 0.5f;
    sBOOMAnomalyConfig.FallingThreshold = 0.5f;
    sBOOMAnomalyConfig.Timeout = COUNT_TO_5S_ON_100Khz;
    Anomaly_Detection_Init(&sBOOMAnomalyConfig);
}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs){
    ...
    if(CAN_ReceiveDataRefresh_Flag){
        ...
        sStateBit.Enable_bit      = CAN_RX_Buff[0];
        if(CAN_RX_Buff[2] > 10){
            PowerLimint = CAN_RX_Buff[2] - PBAT_POWER_LOSS;
        }else {
            PowerLimint = 10;
        }
...
void DebugOut_UART(void){
    ...
    sUART_TxData.ch_data[8] = (float)sStateBit.Enable_bit;
    sUART_TxData.ch_data[9] = (float)PowerLimint;
...
static uint32_t ADC_SUM_CNT = 0;//中间计数变量
/**
